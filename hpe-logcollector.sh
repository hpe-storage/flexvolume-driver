...
if [[ ! -z $node_name ]]; then
	...
	if [[ -z $node_from_kubectl ]]; then
	  ...
	  pod_name=$(kubectl get pods -n $namespace --selector=name=hpe-flexvolume-driver --field-selector=spec.nodeName=$node_from_kubectl -o json -o jsonpath='{.items..metadata.name}')
	  # hpe-flexvolume-driver pod may not be running on this node.
	  if [[ ! -z $pod_name ]]; then
	     kubectl exec -it $pod_name -n $namespace -- hpe-logcollector.sh;
	  else
	     echo "hpe-flexvolume-driver pod is not running on node=$node_from_kubectl in namepspace=$namespace. Please specify valid node/namespace on which hpe-flexvolume-driver is running."
	  fi
	fi
else
	# collect the diagnostic logs from all the nodes
	for i in `kubectl get pods -n $namespace --selector=name=hpe-flexvolume-driver -o json -o jsonpath='{.items..metadata.name}'` ;
	do
		kubectl exec -it $i -n $namespace -- hpe-logcollector.sh;
	done;
fi
