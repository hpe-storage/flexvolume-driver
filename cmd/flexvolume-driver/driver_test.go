...
var basicTests = []struct {
	...
	{"test/good", true, "/run/docker/plugins/nimble.sock", true, "/var/log/hpe-flexvolume-driver.log", "info", true, false, 1073741824, []string{"size", "sizeInGiB"}, true},
	{"test/flipped", true, "nimble", false, "some path", "trace", false, true, 14, []string{"size", "sizeInGiB", "w", "x", "y", "z"}, false},
	{"test/broken", false, "/run/docker/plugins/nimble.sock", true, "/var/log/hpe-flexvolume-driver.log", "info", true, false, 1073741824, []string{"size", "sizeInGiB"}, true},
	{"test/errors", true, "21", true, "true", "info", true, false, 1073741824, []string{"size", "sizeInGiB"}, true},
}
...
func TestConfigFiles(t *testing.T) {
	for _, tc := range basicTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if logLevel != tc.logLevel {
				t.Error(
					"For", "logLevel",
					"expected", tc.logLevel,
					"got:", logLevel,
				)
			}
