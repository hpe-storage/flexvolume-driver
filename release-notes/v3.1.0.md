...
The release notes describe the major changes, fixes, and known issues for this release of the new HPE Volume Driver for Kubernetes FlexVolume Plugin 3.1.0. They do not include all individual fixes and internal changes.
...
|ID|Component |Title|Description|Workaround|
|--|---------|-----|-----------|----------|
|CON-538|flexvolume|Duplicate bind mount paths on host|After the host path mount is done by FlexVolume driver, kubelet bind mounts this path into pod namespace. Often we have seen duplicate bind mount entries(/var/lib/kubelet/pods...) in /proc/mounts or mount output| No action necessary, as no functionality is affected. All bindmounts are cleaned up during unmount automatically |
|CON-527|flexvolume|Pod is stuck in creation state for long time |Our FlexVolume driver checks for /dev/disk/by-path symlinks to verify logged-in iSCSI targets. If Pods were moved back and forth between nodes, there can be stale entries for that iSCSI target pointing to non-existent device. FlexVolume Driver incorrecty assumes that iSCSI target is logged-in for new volume and avoids new login. Thus mount will not suceed and pod is stuck in Creation state| This is seen very rarely when pods are moved around between nodes. Workaround is to either reboot the node or manually cleanup stale symlinks under /dev/disk/by-path for the iSCSI target in error |
|CON-529|flexvolume|Node stuck after reboot| After node reboot, certain times it takes long time come up.| No workaround at this point, as we believe its mostly to do with kubelet issues. Pods will be failed over to another node in this case without issues.|
|CON-456|flexvolume|iSCSI connectivity failures with multiple interfaces |When multiple interfaces are configured on each node for iSCSI traffic, if discovery IP is not reachable by both, then FlexVolume driver will take long time during mount requests | No action necessary, as mount will be successful using at-least one good connection. But path redundancy will be lost across different interfaces |
